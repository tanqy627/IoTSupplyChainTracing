...
Cross-dimensional classification of all 186 released groups (36 Dimension I,
69 Dimension II, and 81 Dimension III groups).

Columns:

- `Source`: dimension that produced the group (`D1`, `D2`, or `D3`).
- `Category`: one of the seven dimensional-overlap categories.
- `Num_Devices`, `Devices`: group size and pipe-separated device list.

### `overlap_summary.csv`

Aggregate counts for the seven Table VI overlap categories. Columns are
`Category` and `Groups`.
