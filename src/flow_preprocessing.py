...
def load_device_types(path: Optional[str]) -> Dict[str, str]:
    ...
    return dict(zip(df["Device_Name"].map(canonicalize_device_name), df["Type"]))
...
def load_vendor_map(path: Optional[str]) -> Dict[str, str]:
    ...
    return dict(
        zip(
            df["Device_Name"].astype(str).map(canonicalize_device_name),
            df["Vendor"].fillna("unknown").astype(str).str.lower(),
        )
    )
...
def build_pcap_tasks(
    ...
    for root_dir in root_dirs:
        for root, _, files in os.walk(root_dir):
            ...
            if rel == ".":
                continue
            device_name = canonicalize_device_name(rel.split(os.sep)[0])
...
def filter_flows(
    ...
    for flow in tqdm(flows, desc="Filtering"):
        ...
        if vendor_map:
            flow = dict(flow)
            flow["Device"] = canonicalize_device_name(flow.get("Device", ""))
            flow["Vendor"] = vendor_map.get(flow["Device"], "unknown")
        cleaned.append(flow)
