...
A schema file is provided at:

```text
data/files_with_suggested_ips.schema.csv
```

The public repository should only include this schema. Do not publish a real
mapping file if it contains private local paths, local IP addresses, or other
sensitive environment-specific identifiers.

## Device Metadata

The preprocessing and analysis scripts also use local device metadata files.
These files are not included as toy examples in the first-stage release, but
their required columns are documented in `docs/input_schema.md`. In the
measurement pipeline, the device list maps devices to vendors, while the device
type file maps devices to coarse device categories. They are used for
vendor-level grouping, non-target-device filtering, and group summaries.
