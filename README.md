...
docs/
  ...
  input_schema.md             Required input columns
```
...
python src/flow_preprocessing.py extract \
  ...
  --manual-ip-csv data/files_with_suggested_ips.csv \
...
python src/endpoint_matching.py \
  ...
  --group-output-csv outputs/dim2_groups.csv
...
python src/behavior_clustering.py export-groups \
  ...
  --output-csv outputs/dim3_all_clusters.csv

# 8. Corroboration
python src/corroboration.py \
  --global-component-csv outputs/global_components.csv \
  --dim3-groups-csv outputs/dim3_all_clusters.csv \
  --dim1-groups-csv outputs/dim1_groups.csv \
  --dim2-groups-csv outputs/dim2_groups.csv \
  --output-csv outputs/corroboration_results.csv
```

## Notes

- Dimension I is exposed as prompts and deterministic evidence normalization
  rather than a mandatory LLM API dependency.
...
additional preparation notes in `docs/dataset.md`. The repository only includes
schema files and code for processing locally obtained datasets.
